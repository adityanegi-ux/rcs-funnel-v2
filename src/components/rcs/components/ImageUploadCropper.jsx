...
function ImageUploadCropper({ spec, value, onChange, onDone }) {
  ...
  return (
    <div className='h-full'>
      {cropState ? (
        <div className='h-full rounded-xl border border-[#D0D5DD] bg-white p-4 flex flex-col'>
          ...
          <div className='mt-3 flex-1 min-h-0 rounded-lg border border-[#EAECF0] bg-[#F8FAFC] p-2 flex items-center justify-center'>
            <img
        ...
        <div className='h-full rounded-xl border border-[#D0D5DD] bg-white p-4'>
          <h4 className='text-base font-semibold text-[#111827]'>{spec.title}</h4>
          <p className='text-xs text-[#667085] mt-1'>
            Minimum: {spec.minWidth}x{spec.minHeight} | Output: {spec.outputWidth}x{spec.outputHeight}
          </p>
